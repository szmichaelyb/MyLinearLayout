...
-(void)loadView
{
    /*
     ...
     1.布局视图：    就是从MyBaseLayout派生而出的不同的类型的视图，目前MyLayout中一共有：线性布局、框架布局、相对布局、表格布局、流式布局、浮动布局、路径布局、栅格布局、弹性布局9种布局。 布局视图也是一个视图，因为MyBaseLayout也是从UIView派生的。
     2.非布局视图：  除上面说的9种布局视图外的所有视图和控件。
     ...
     再次强调的是：
     1. 如果同时设置了左右边距并且在尺寸不是自适应时就能决定自己的宽度，同时设置左右间距不能决定自己的宽度！
     2. 如果同时设置了上下边距并且在尺寸不是自适应时就能决定自己的高度，同时设置上下间距不能决定自己的高度！
